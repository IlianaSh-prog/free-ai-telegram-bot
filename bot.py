...
client = OpenAI(api_key=PROXYAPI_KEY, base_url="https://api.proxyapi.ru/v1")
...
def log_payment(user_id, username, amount):
  with db_lock:
    ...
    c = conn.cursor()
    c.execute(
        "INSERT INTO payments (user_id, username, amount) VALUES (?, ?, ?)",
...
def get_main_keyboard(user_id):
  ...
  btn_video = types.InlineKeyboardButton(
      "🎬 Создать видео из фото (до 5 шт)", callback_data="act_create_video"
  )
  btn1 = types.InlineKeyboardButton(
      "🔥 Факты / Топы", callback_data="genre_facts"
  )
  btn2 = types.InlineKeyboardButton(
      "💡 Экспертный / Польза", callback_data="genre_expert"
  )
  btn3 = types.InlineKeyboardButton(
      "😱 Мистика / Истории", callback_data="genre_story"
  )
  btn4 = types.InlineKeyboardButton(
      "💰 Деньги / Бизнес", callback_data="genre_business"
  )
  btn_buy = types.InlineKeyboardButton(
      f"⭐ Купить 20 генераций (Баланс: {credits_left})",
      callback_data="buy_credits",
  )
  btn_rules = types.InlineKeyboardButton(
      "📄 Правила использования", url="https://telegra.ph"
  )
...
def handle_start(message):
  ...
  text = (
      ...
      "• 🎬 **Монтаж видео:** пришлите от 1 до 5 фото (можно с текстом в"
      " подписи)!\n"
...
def handle_admin_stats(message):
  ...
  with db_lock:
    ...
    c.execute(
        "SELECT COUNT(DISTINCT user_id), COUNT(*), SUM(amount) FROM payments"
    )
    row = c.fetchone()
...
def handle_incoming_photos(message):
  ...
  if credits_left <= 0:
    bot.reply_to(
        message,
        "⛔ У вас закончились генерации. Пополните баланс.",
        reply_markup=get_main_keyboard(user_id),
    )
    return
  ...
  bot.reply_to(
      ...
      "Отправьте ещё фото или напишите текст для диктора (можно слово"
      " **Готово**).",
      parse_mode="Markdown",
  )
  ...
  try:
    ...
    with open(voice_filename, "rb") as audio_file:
      audio_msg = bot.send_audio(
          chat_id, audio_file, caption="🎙 Подготовка дорожки..."
      )
    ...
    elements.append({"type": "audio", "track": 2, "source": audio_url})
    ...
    if resp.status_code in [200, 201, 202]:
      ...
      if video_url:
        ...
        bot.send_video(
            ...
            video_url,
            caption=(
                f"🎬 **Готовое видео из {count_photos} фото с озвучкой!**"
            ),
            parse_mode="Markdown",
        )
        bot.send_message(
        ...
        bot.edit_message_text(
            "❌ Рендер видео занял больше времени. Попробуйте ещё раз.",
            chat_id,
            status_msg_id,
        )
    else:
      bot.edit_message_text(
          f"❌ Ошибка Creatomate: {render_res}", chat_id, status_msg_id
      )

  except Exception as e:
    bot.edit_message_text(
        f"❌ Ошибка сборки видео: {e}", chat_id, status_msg_id
    )
  finally:
...
def handle_all_text_messages(message):
  ...
  if user_id in user_media_data and user_media_data[user_id]["photos"]:
    ...
    if text.lower() == "готово":
      script_text = (
          user_media_data[user_id].get("caption")
          or "Посмотрите на эти кадры вокруг нас."
      )
    else:
      script_text = text
    ...
    status_msg = bot.reply_to(
        message,
        f"🎬 *Монтируем видео из {len(photos_list)} фото с озвучкой...*",
        parse_mode="Markdown",
    )
    assemble_video(
  ...
  if credits_left <= 0:
    bot.reply_to(
        message,
        "⛔ **Баланс исчерпан.** Пополните баланс звёздами ниже.",
        reply_markup=get_main_keyboard(user_id),
    )
    return
  ...
  status_msg = bot.reply_to(
      message, "⏳ *ИИ анализирует тренды и пишет сценарий...*", parse_mode="Markdown"
  )

  system_prompt = (
      "Ты — профессиональный продюсер вирусных коротких видео (YouTube"
      " Shorts, Instagram Reels, TikTok). Твоя цель — создать сценарий с"
      " удержанием 100%. Структура ответа строго:\n1. 🎯 ВИРУСНЫЙ ХУК (первые 3"
      " сек);\n2. 📜 СЦЕНАРИЙ (до 45 сек, разбит по секундам [0-5], [5-15] и"
      " т.д.);\n3. 🎨 ВИЗУАЛЬНЫЙ РЯД (покадровый план + детальный промпт на"
      " английском);\n4. 🎵 МУЗЫКА (темп, жанр);\n5. 🏷 ХЭШТЕГИ (5-7 штук)."
  )
  ...
  try:
    ...
    bot.edit_message_text(
        f"❌ Ошибка генерации: {e}", message.chat.id, status_msg.message_id
    )
...
def handle_callback(call):
  ...
  if call.data == "act_create_video":
    bot.send_message(
        ...
        "📸 **Отправьте от 1 до 5 фотографий!**\nВы можете прислать их"
        " альбомом или по одной. После этого напишите текст для диктора.",
        parse_mode="Markdown",
    )
    ...
    prices = [
        types.LabeledPrice(
            label="20 генераций Shorts/Reels", amount=PACKAGE_PRICE_STARS
        )
    ]
    bot.send_invoice(
